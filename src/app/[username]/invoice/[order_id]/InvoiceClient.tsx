...
export default function InvoiceClient({ invoice }: { invoice: any }) {
  ...
  const subtotal = invoice.items.reduce((s: number, item: any) => s + (item.qty * item.price), 0);
  
  const sellerPhone = invoice.storeWhatsapp;
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden relative">
          ...
          <div className="p-8 pb-6 flex flex-col items-center">
            <div className="w-16 h-16 rounded-2xl bg-surface-container border-2 border-outline-variant/20 overflow-hidden relative shadow-sm mb-4">
              <Image src={invoice.storeLogo || "https://placehold.co/400x400/eeeeee/999999"} alt={invoice.storeName} fill className="object-cover" unoptimized />
            </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg overflow-hidden">
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-200/50 p-3.5 rounded-2xl">
              ...
              <div>
                <p className="text-[11px] font-bold text-amber-800">Status Pesanan:</p>
                <p className="text-xs font-black text-amber-900">{invoice.statusText}</p>
              </div>
            </div>
                ...
                {invoice.items.map((item: any, idx: number) => (
                  <div key={idx} className="flex justify-between items-start gap-4 bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant/10">
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-on-surface-variant font-medium mt-0.5">{item.qty}x</p>
                    </div>
