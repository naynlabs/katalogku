...
export default function AudiensClient({ initialAudience }: AudienceProps) {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredAudience = initialAudience.filter((p) =>
    (p.email || p.phone || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/20 tonal-depth">
          ...
          <p className="text-4xl font-black text-on-surface">{initialAudience.length}</p>
          <p className="text-xs text-green-600 font-bold mt-2 flex items-center gap-1">
            <span className="material-symbols-outlined text-[14px]">trending_up</span> Terbaru
          </p>
        </div>
        <div className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/20 tonal-depth">
          <div className="flex items-center gap-3 mb-2">
