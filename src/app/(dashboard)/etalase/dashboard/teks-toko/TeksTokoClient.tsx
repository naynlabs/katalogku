...
const defaultTeksConfig = {
  waTemplate: "Halo {{nama_toko}}! 🛍️\n\nSaya tertarik dan ingin memesan:\n{{daftar_produk}}\n\n*Total Belanja: {{total_harga}}*\n\nApakah stoknya masih tersedia? Terima kasih!",
  btnCartAdd: "Tambah",
  btnCartAdded: "✓ Ditambahkan",
  btnFloatingCart: "Lihat Keranjang",
  btnCheckout: "Pesan via WhatsApp",
  noteCart: "Pesanan akan dikirim langsung ke WhatsApp penjual",
  emptyCart: "Belanjaan belum ada, mulai tambahkan keranjang Anda."
};

export default function TeksTokoClient({ initialConfig, storeName = "Butik Clarissa" }: { initialConfig: any; storeName: string }) {
  ...
  const handleSave = () => {
    startTransition(async () => {
      ...
      if (res.error) {
        ...
        alert("Pengaturan Teks Toko berhasil disimpan!");
      }
    });
  };
          ...
          <section className="bg-surface-container-lowest rounded-2xl p-6 lg:p-8 border border-outline-variant/20 tonal-depth">
            ...
            <div className="flex flex-col mb-4">
              ...
              <textarea 
                ...
                onChange={(e) => handleUpdate('waTemplate', e.target.value)}
                rows={7} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={config.btnCartAdd} onChange={(e) => handleUpdate('btnCartAdd', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl pl-10 pr-4 py-3 min-h-[48px] text-sm font-bold text-on-surface transition-colors outline-none" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={config.btnCartAdded} onChange={(e) => handleUpdate('btnCartAdded', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl pl-10 pr-4 py-3 min-h-[48px] text-sm font-bold text-on-surface transition-colors outline-none" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={config.btnFloatingCart} onChange={(e) => handleUpdate('btnFloatingCart', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl pl-10 pr-4 py-3 min-h-[48px] text-sm font-bold text-on-surface transition-colors outline-none" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={config.btnCheckout} onChange={(e) => handleUpdate('btnCheckout', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl pl-10 pr-4 py-3 min-h-[48px] text-sm font-bold text-on-surface transition-colors outline-none" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-surface-container-lowest rounded-2xl p-6 lg:p-8 border border-outline-variant/20 tonal-depth">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input type="text" value={config.noteCart} onChange={(e) => handleUpdate('noteCart', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl px-4 py-3 min-h-[48px] text-sm font-medium text-on-surface transition-colors outline-none" />
              </div>
              
              <div>
                <label className="block text-xs font-bold text-on-surface-variant mb-2">Detail Kosong (Placeholder)</label>
                <input type="text" value={config.emptyCart} onChange={(e) => handleUpdate('emptyCart', e.target.value)} className="w-full bg-surface-container-low focus:bg-white border-2 border-transparent focus:border-primary rounded-xl px-4 py-3 min-h-[48px] text-sm font-medium text-on-surface transition-colors outline-none" />
              </div>
            </div>
          </section>
            ...
            <div className="border-[6px] border-[#128C7E] rounded-[2.5rem] w-[320px] h-[480px] bg-[#E5DDD5] mx-auto overflow-hidden shadow-2xl relative flex flex-col">
              ...
              <div className="bg-[#075E54] text-white px-4 py-3 flex items-center gap-3 shadow-md z-10">
                ...
                <div className="flex-1 overflow-hidden">
                  <h4 className="font-bold text-sm truncate leading-tight">Admin {storeName}</h4>
