...
import { syncLinks, syncSocialLinks } from "@/lib/actions";
...
type Props = {
  ...
  initialSocials: SocialFromDB[];
  storeProducts: ProductPreview[];
  storeProfile: {
    name: string;
    tagline: string;
    wa: string;
...
export default function LinksClient({
  ...
  // Hydrate Zustand store from DB data on first mount
  useEffect(() => {
    if (!hydrated.current) {
      hydrated.current = true;
      // Hydrate profile from DB
      if (storeProfile.name) store.setProfileField("name", storeProfile.name);
      if (storeProfile.tagline) store.setProfileField("tagline", storeProfile.tagline);
      if (storeProfile.banner) store.setProfileField("banner", storeProfile.banner);
      if (storeProfile.profile) store.setProfileField("profile", storeProfile.profile);
      store.setProfileField("isVerified", storeProfile.isVerified);

      // Hydrate links from DB
      if (initialLinks.length > 0) {
        store.reorderLinks(initialLinks as any);
      }
      // Hydrate socials from DB
      if (initialSocials.length > 0) {
        // Clear existing and set from DB
        initialSocials.forEach((s) => {
          if (!store.socials.find((x) => x.id === s.id)) {
            store.addSocial({ id: s.id, platform: s.platform, handle: s.handle, url: s.url });
          }
        });
      }
    }
  ...
  const previewData = {
    ...
    categories: [...new Set(storeProducts.map((p) => p.cat))],
    ...
    startTransition(async () => {
      try {
        ...
        await syncSocialLinks(
          store.socials.map((s, i) => ({
            platform: s.platform,
            handle: s.handle || s.platform,
            url: s.url,
            color: undefined,
            sortOrder: i,
          }))
        ...
        <div className="w-full h-full flex flex-col items-center overflow-y-auto hide-scrollbar pt-8 pb-24">
          ...
          >
             <div className="w-full h-full relative">
                <StorefrontUI data={previewData as any} disableCheckout={true} />
             </div>
          </div>
        </div>
      </div>
         ...
         <div className="flex flex-col gap-6 animate-fade-in">
            ...
            <section className="mt-4 border-t border-outline-variant/20 pt-6 flex flex-col gap-2">
              ...
              >
                ...
                {saveStatus === "saving" ? "Menyimpan..." : saveStatus === "saved" ? "Tersimpan!" : "Simpan Desain"}
              </button>
            </section>
