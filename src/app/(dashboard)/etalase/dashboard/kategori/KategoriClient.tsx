...
type CategoryItem = {
  id: string; // The db id is number but comes as string for client convenience here
  name: string;
  count: number;
  enabled: boolean;
};
...
export default function KategoriClient({
  ...
  const onAddCategory = (data: AddCategoryFormData) => {
    // Optimistic-like UI could be added, but simple server action is fine
    startTransition(async () => {
      await createCategory(data.name);
      // Wait for server to revalidate Path, ideally we trust server or fetch again
      // The page will revalidate and provide new props
      reset();
    });
  };
  ...
  const handleSaveEdit = (id: string, newName: string) => {
    ...
    setEditingId(null);
    startTransition(async () => {
      await updateCategory(parseInt(id), newName);
    });
  };

  const toggleStatus = (id: string) => {
    // The schema doesn't have isActive, so this is just UI for now
    // Future: Add isActive to product_categories table
    setCategories((prev) =>
  ...
  const executeDelete = () => {
    if (categoryToDelete) {
      ...
      startTransition(async () => {
        await deleteCategory(parseInt(categoryToDelete.id));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
        ...
        <div className="lg:col-span-4 space-y-6">
          <form
            ...
            className="bg-surface-container-lowest p-6 rounded-3xl border border-outline-variant/20 tonal-depth"
          >
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  {...register("name")}
                  disabled={isPending}
                  className={`w-full bg-surface-container-low border-2 ${
                    errors.name ? "border-error" : "border-transparent"
                  } focus:bg-white focus:border-primary rounded-xl px-4 py-3 text-sm text-on-surface font-medium outline-none transition-colors`}
                />
                {errors.name && (
                  <p className="text-error text-xs font-bold mt-1 ml-1">{errors.name.message}</p>
                )}
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full py-3.5 bg-primary text-white rounded-full font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className="divide-y divide-outline-variant/10">
              {initialCategories.length > 0 ? (
                initialCategories.map((cat, index) => (
                  <div
                    key={cat.id}
                    className={`p-5 flex items-center justify-between hover:bg-surface-container-lowest transition-colors bg-white group ${isPending ? 'opacity-70' : ''}`}
                  >
                    <div className="flex items-center gap-4 flex-1">
                      {/* Drag Handle Dummy */}
                      <span className="material-symbols-outlined text-outline-variant opacity-30 cursor-grab hover:opacity-80 active:cursor-grabbing">
                        drag_indicator
                      </span>
                      ...
                      <div className="flex-1">
                        {editingId === cat.id ? (
                          <div className="flex gap-2 w-full max-w-sm">
                            ...
                            />
                            <button
                              onClick={() => handleSaveEdit(cat.id, editingName)}
                              className="bg-primary text-white p-1 rounded-md hover:bg-primary-fixed disabled:opacity-50"
                              disabled={isPending}
                            >
                            ...
                            <button
                              ...
                              className="bg-surface-container text-on-surface-variant p-1 rounded-md hover:bg-surface-container-high"
                            >
                          ...
                          <>
                            <h4
                              className={`font-bold text-[15px] mb-0.5 ${
                                !cat.enabled ? "text-on-surface-variant line-through" : "text-on-surface"
                              }`}
                            >
                              {cat.name}
                            </h4>
                    ...
                    <div className="flex items-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className={`w-12 h-6 rounded-full relative transition-colors ${
                          cat.enabled ? "bg-primary" : "bg-outline-variant/40"
                        }`}
                      ...
                      <button
                        ...
                        title="Edit Nama"
                        disabled={editingId === cat.id || isPending}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-surface-container-lowest max-w-sm w-full rounded-[2rem] p-8 shadow-2xl relative z-10 animate-scale-in">
            ...
            <p className="text-center text-sm text-on-surface-variant mb-8 leading-relaxed">
              ...
              <span className="font-bold text-on-surface">{categoryToDelete.name}</span>? Produk
              di dalam kategori ini tidak akan terhapus, namun tidak akan memiliki kategori lagi di
              etalase Anda.
            </p>
