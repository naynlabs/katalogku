...
// --- REAL DATA STRUCTURE (Drawn from Props) ---
interface AnalyticsProps {
  weeklyData: { day: string; views: number; clicks: number }[];
  monthlyData: { day: string; views: number; clicks: number }[];
}
...
function AreaChart({ data, width = 600, height = 220 }: { data: AnalyticsProps["weeklyData"]; width?: number; height?: number }) {
  const maxViews = Math.max(...data.map(d => d.views));
  const maxClicks = Math.max(...data.map(d => d.clicks));
  const maxVal = Math.max(maxViews, maxClicks, 1); // ensure no division by zero
...
// ---- Main Page ----
export default function AnalyticsClient({ weeklyData, monthlyData }: AnalyticsProps) {
  const [period, setPeriod] = useState<'7d' | '30d'>('7d');
  const activeData = period === '7d' ? weeklyData : monthlyData;
