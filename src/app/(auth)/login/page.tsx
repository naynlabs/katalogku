...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    if (result.error) {
      setAuthError(result.error.message || "Email atau kata sandi salah. Silakan coba lagi.");
      return;
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-on-surface-variant">
            Masuk ke akun Anda untuk mulai mengelola katalog digital hari ini.
          </p>
        </div>
        ...
        <div className="bg-surface-container-lowest rounded-xl p-8 shadow-[0px_20px_40px_rgba(77,68,227,0.06)] relative overflow-hidden">
          ...
          <div className="flex bg-surface-container-low p-1 rounded-full mb-8">
            ...
            <Link href="/register" className="flex-1 py-2 text-sm font-semibold rounded-full text-on-surface-variant hover:text-on-surface transition-all text-center">
              Daftar
            </Link>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-full border border-outline-variant bg-white hover:bg-surface-container-low transition-colors mb-6 group disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isGoogleLoading ? (
        ...
        <p className="text-center mt-8 text-sm text-on-surface-variant">
          Belum punya akun?{" "}
          <Link className="text-primary font-bold hover:underline" href="/register">Ayo mulai jualan hari ini</Link>
        </p>
      </div>
    </main>
  );
